...
- server from IBM(512 GB RAM, 1000 CPUs huge configurations) ->
  - hypervisor sw (logically divide/isolated the server into multiple servers called virtual machines)
    - dev1 requests one server -> instead of giving access to IBM server, will give one virtual machine as per required configurations.
...
## Elasticity (dynamic/auto scalling)
